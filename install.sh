...
if [ -n "${KIMI_API_KEY:-}" ]; then
    ...
    warn "KIMI_API_KEY is not set"
    echo ""
    key=$(ask_text "Enter your Kimi API key (will be saved to shell config):")
    ...
    echo "" >> "$rc_file"
    echo "# Kimi API key (added by claudimi installer)" >> "$rc_file"
    echo "export KIMI_API_KEY=\"${key}\"" >> "$rc_file"
...
chmod +x "$tmp"
mv "$tmp" "${INSTALL_DIR}/claude-kimi"
ok "Installed to ${INSTALL_DIR}/claude-kimi"
...
if [ -d "$CLAUDE_PROJECTS" ]; then
    if ask "Link project memories from ${CLAUDE_PROJECTS}? (shared between Claude and claudimi)"; then
        mkdir -p "$CONFIG_DIR"
...
echo ""
if ask "Install the claude-kimi-teammate skill?"; then
    if command -v npx &>/dev/null; then
        ...
        echo "  npx skills add nathabonfim59/claudimi -a claude-code -g -y"
    fi
else
