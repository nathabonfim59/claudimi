---
name: claude-kimi-teammate
description: Spawn `claude-kimi` as an interactive teammate in a tmux window so the user can watch/steer it live, while Claude and claude-kimi message each other via `tmux send-keys`. Use this whenever a user asks to delegate implementation to claude-kimi, or whenever the memory rule "delegate implementation to claude-kimi" applies and an interactive session is useful.
---
...
- **Teammate pane**: a new tmux window in the same session running `claude-kimi` interactively.
...
1. Verify I'm in tmux: require `$TMUX` and `$TMUX_PANE` to be set. If not, fall back to `claude-kimi -p` background mode and tell the user.
2. Pick a window name based on the task (e.g. `kimi-video`, `kimi-auth`). Keep it short, prefix with `kimi-`.
3. Create the window in the *current* session (use `-c <cwd>` so it starts in the project dir):

   ```bash
   MY_PANE="$TMUX_PANE"
   WIN=kimi-<taskslug>
   tmux new-window -d -n "$WIN" -c "$PWD" 'claude-kimi'
   sleep 5   # claude-kimi needs ~4-5s to boot and render UI
...
5. Send the initial briefing — it MUST include (a) the task, (b) my pane id so it can message me, (c) the explicit messaging protocol. Use `tmux send-keys -l` (literal) for the body, then a separate `Enter`:
   ...
   e.g. "kimi-video"). EVERY message you send back MUST start with
...
6. Record the pane id somewhere stable for later sends. Simplest: save to `/tmp/claude-kimi-teammate-<slug>.pane` — one line with the pane id.

## Sending follow-up messages

```bash
TEAMMATE_PANE=$(cat /tmp/claude-kimi-teammate-<slug>.pane)
...
- If `$TMUX` is unset, this skill cannot run; fall back to `claude-kimi -p` in background and explain why.
